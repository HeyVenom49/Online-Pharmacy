...
public class MedicineController {
    ...
    public ResponseEntity<ApiPaginatedResponse<List<MedicineDTO>>> getAllMedicines(
            @PageableDefault(size = 20) Pageable pageable) {
        Page<MedicineDTO> page = medicineService.getAllMedicines(pageable);
        return ResponseEntity.ok(ApiPaginatedResponse.of(page.getContent(), page));
    }

    @GetMapping("/search")
    @Operation(summary = "Search medicines", description = "Search medicines with filters")
    public ResponseEntity<ApiPaginatedResponse<List<MedicineDTO>>> searchMedicines(
        ...
        Page<MedicineDTO> page = medicineService.searchMedicines(request, pageable);
        return ResponseEntity.ok(ApiPaginatedResponse.of(page.getContent(), page));
    }
    ...
    public ResponseEntity<ApiResponse<MedicineDetailDTO>> getMedicineById(@PathVariable Long id) {
        return ResponseEntity.ok(ApiResponse.of(medicineService.getMedicineById(id)));
    }

    @PostMapping
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Create medicine", description = "Creates a new medicine (Admin only)")
    public ResponseEntity<ApiResponse<MedicineDTO>> createMedicine(@Valid @RequestBody MedicineRequest request) {
        MedicineDTO medicine = medicineService.createMedicine(request);
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.of(medicine, "Medicine created successfully"));
    }
    ...
    public ResponseEntity<ApiResponse<MedicineDetailDTO>> createMedicineWithInventory(
        ...
        return ResponseEntity.status(HttpStatus.CREATED)
                .body(ApiResponse.of(medicine, "Medicine created with inventory"));
    }
    ...
    public ResponseEntity<ApiResponse<MedicineDTO>> updateMedicine(
        ...
        return ResponseEntity.ok(ApiResponse.of(medicine, "Medicine updated successfully"));
    }

    @DeleteMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Delete medicine", description = "Permanently deletes a medicine (Admin only)")
    public ResponseEntity<ApiResponse<Void>> deleteMedicine(@PathVariable Long id) {
        medicineService.deleteMedicinePermanently(id);
        return ResponseEntity.ok(ApiResponse.of(null, "Medicine deleted successfully"));
    }

    @PatchMapping("/{id}/activate")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Activate medicine", description = "Reactivates a soft-deleted medicine (Admin only)")
    public ResponseEntity<ApiResponse<MedicineDTO>> activateMedicine(@PathVariable Long id) {
        MedicineDTO medicine = medicineService.activateMedicine(id);
        return ResponseEntity.ok(ApiResponse.of(medicine, "Medicine activated successfully"));
    }

    @PatchMapping("/{id}/deactivate")
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Deactivate medicine", description = "Deactivates a medicine (Admin only)")
    public ResponseEntity<ApiResponse<MedicineDTO>> deactivateMedicine(@PathVariable Long id) {
        MedicineDTO medicine = medicineService.deactivateMedicine(id);
        return ResponseEntity.ok(ApiResponse.of(medicine, "Medicine deactivated successfully"));
    }
}

        ...
        try {
            if (event.getReservations() != null) {
                for (InventoryReservedEvent.ReservationItem item : event.getReservations()) {
                    boolean success;
                    // If reservationId starts with "RES-", it's a confirmed order - deduct stock permanently
                    if (event.getReservationId() != null && event.getReservationId().startsWith("RES-")) {
                        success = inventoryService.deductStock(item.getMedicineId(), item.getQuantity());
                        if (success) {
                            log.info("Successfully deducted stock: medicineId={}, quantity={}, orderId={}", 
                                    item.getMedicineId(), item.getQuantity(), event.getOrderId());
                        } else {
                            log.error("Failed to deduct stock: medicineId={}, quantity={}, orderId={}", 
                                    item.getMedicineId(), item.getQuantity(), event.getOrderId());
                        }
                    } else {

...
public class CatalogFeignClientFallbackFactory implements FallbackFactory<CatalogFeignClient> {
    ...
    public CatalogFeignClient create(Throwable cause) {
        ...
        return new CatalogFeignClient() {
            ...
            @Override
            public Boolean reserveStock(Long medicineId, Integer quantity, String reservationId) {
                log.warn("Fallback: reserveStock for medicine {} quantity {}", medicineId, quantity);
                return false;
            }

            @Override
            public Boolean releaseStock(Long medicineId, Integer quantity, String reservationId) {
                log.warn("Fallback: releaseStock for medicine {} quantity {}", medicineId, quantity);
                return false;
            }
        };
    }
